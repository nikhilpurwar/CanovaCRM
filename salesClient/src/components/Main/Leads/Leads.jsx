...
import { fetchMyLeads, updateLead as updateLeadAction } from '../../../redux/leadsSlice'
import './Leads.css'
...
const Leads = () => {
    ...
    useEffect(() => {
        dispatch(fetchMyLeads({ page: 1, limit: 50 }));
    }, [dispatch]);
    ...
    const handleTypeSelect = (val) => {
        ...
        dispatch(updateLeadAction({ id: selectedLead._id, data: { type: val } }));
        setTypeOpen(false);
    }
    ...
    const handleDateSave = ({ date, time }) => {
        ...
        dispatch(updateLeadAction({ id: selectedLead._id, data: { scheduledDate: combined } }));
        setDateOpen(false);
    }
    ...
    const handleStatusSave = (val) => {
        ...
        if (val === 'Closed' && selectedLead.scheduledDate) {
            return
        }
        dispatch(updateLeadAction({ id: selectedLead._id, data: { status: val } }));
        setStatusOpen(false);
    }
            ...
            <div className="lead-list">
                {leads.length === 0 ? (
                    <div style={{ padding: '40px 20px', textAlign: 'center', color: '#999' }}>
                        {loading ? 'Loading leads...' : 'No leads assigned yet'}
                    </div>
                ) : (
                    leads.map((lead) => (
                        <div className={`lead-card ${lead.highlight ? 'highlight' : ''}`} key={lead._id || lead.id}>
                            <span className={`status-bar ${getStatusColor(lead.status)}`} />
                            ...
                            <div className="lead-info">
                                ...
                                <div className="date">
                                    ...
                                    <span>{formatDate(lead.leadDate || lead.date)}</span>
                                </div>
                            </div>

                            <div className="lead-actions">
                                <div className={`status-circle ${getStatusColor(lead.status)}`}>
                                    {lead.status}
                                </div>
                                ...
                                <div className="icons">
                                    ...
                                    <svg onClick={() => openStatusFor(lead)} style={{cursor:'pointer'}} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <path fillRule="evenodd" clipRule="evenodd" d="M9.33344 13.7217L4 8.34415L5.33312 7L10 11.7055L14.6669 7L16 8.34415L10.6666 13.7217C10.4898 13.8999 10.25 14 10 14C9.75 14 9.51024 13.8999 9.33344 13.7217Z" fill="#2051E5" />
                                    </svg>
                                </div>
                            </div>
                        </div>
